import { useGetAllOrderQuery } from "src/redux/rtkQuery/order";
import SpinLoading from "../spinner/spinLoading";
import ItemShip from "./item";

const Logistics: React.FC = () => {
    const { data: orders, isLoading } = useGetAllOrderQuery();
    const listOrderShipping = orders?.filter((order: any) => order.statusShipping === "Đã gửi hàng" || order.statusShipping === "Đã giao hàng");
    return (
        <>
                            ...
                            <table className="min-w-full border-collapse border-spacing-y-2 border-spacing-x-2">
                                <thead className="hidden border-b lg:table-header-group">
                                    <tr className="">
                                        ...
                                        <td className="whitespace-normal py-4 text-sm font-medium text-gray-500 sm:px-3">Địa chỉ giao</td>

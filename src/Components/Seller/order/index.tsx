...
const OrderComponent: React.FC = () => {
    ...
    const findOrder = () =>{
        setListOrderShow((prev) => prev.filter(item => item._id.toLowerCase() === searchOrder.toLowerCase()))
    } 
    return (
        <>
                    ...
                    <div className=' flex items-center'>
                        <div className=' border rounded flex items-center'>
                            ...
                            <div className=' py-1  px-4 '>
                                <input value={searchOrder} onChange={(e)=> setSearchOrder(e.target.value)} type="text" className=' w-[250px]' placeholder=' Nhập Mã đơn hàng' />
                            </div>
                        </div>
                        <div onClick={findOrder} className=' ml-4 text-xl'>
                            <IoSearch />
                        </div>
                    </div>
                </div>

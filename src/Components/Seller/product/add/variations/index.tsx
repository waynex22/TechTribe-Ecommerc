...
const VariationsProduct = ({
  ...
  useEffect(() => {
    ...
    const colorDifferences =
      formAddProduct.variation &&
      variation &&
      formAddProduct.variation["Màu sắc"] &&
      variation["Màu sắc"]
        ? getDifferences(
            formAddProduct.variation["Màu sắc"],
            variation["Màu sắc"]
    ...
    if (formAddProduct.productPrice) {
      ...
      const newPrice = productPrice.map((item) => {
        if (
          ...
          colorDifferences[0] &&
          item.name_color === colorDifferences[0].name
        ) {
          ...
          sizeDifferences.length > 0 &&
          sizeDifferences[0] &&
          item.name_size === sizeDifferences[0].name
        ) {
      ...
      const checkPrice = newPrice.filter(
        (item): item is typeProductPrice => item !== undefined
      );
  ...
  return (
    <div
      ...
      className="px-4 py-8 bg-white rounded-md shadow text-left"
    >
      ...
      <div className=" py-2 flex flex-col gap-6 pt-4 px-12 text-sm font-normal">
        {formAddProduct.variation &&
          Object.keys(formAddProduct.variation).length > 0 && (
            <DetailVariation
              errForm={errForm}
              product={product}

...
const ItemDetailMess = ({message, shop}: {
    message: any
    shop: any
}) => {
    return (
        <>
            {message?.messenger?.map((item: any, index: number) => {
                ...
                return (
                    (
                        <div key={item._id} >
                            ...
                            <div className={` group w-fit items-center ${shop.shopId !== item.id_sender && 'ml-auto '}`}>
                                {item.content && (
                                    <div className={` max-w-[220px] shadow w-fit p-2 rounded ${shop.shopId !== item.id_sender ? 'ml-auto bg-blue-200' : 'bg-white'}`}>
                                    ...
                                    <div className={`w-full p-2 bg-white rounded shadow`}>
                                        ...
                                        <div className=' flex gap-2 items-center text-sm py-1 font-normal'>
                                            ...
                                            <div>
                                                ...
                                                <p className=' text-red-600'> {formatNumberVnd(item.id_product)} </p>

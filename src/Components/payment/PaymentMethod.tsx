...
const PaymentMethod: React.FC<Props> = ({ subOrder, refecth }) => {
    ...
    const { data: wallet } = useGetWalletByCustomerQuery(subOrder?.customerId?._id, {
        skip: !subOrder
    });
    const isNotAvaiLable = (method: string ) => {
        return method == 'Techtribe Pay' && wallet?.balance < subOrder?.total
    }
    const handleSelectPayment = async (method: string) => {
        if(isNotAvaiLable(method)) return
        setLoading(true);
        const payload = {
            id: subOrder?._id,
                ...
                <div className="space-y-8 my-4">
                    <div className="flex items-center">
                        <input
                            type="radio"
                            name="paymentMethod"
                            value={"Thanh toán khi nhận hàng"}
                            checked={"Thanh toán khi nhận hàng" === subOrder?.methodPayment}
                            onChange={() => handleSelectPayment("Thanh toán khi nhận hàng")}
                            className="form-radio h-4 w-4 text-blue-600 transition duration-150 ease-in-out cursor-pointer"
                        />
                        <label htmlFor={"paymentMethod"} className="ml-3 text-sm font-medium text-gray-700 flex gap-2 items-center">
                    ...
                    <div className={`flex items-start ${isNotAvaiLable("Techtribe Pay") ? "opacity-50" : ""} `}>
                        <input
                            type="radio"
                            name="paymentMethod"
                            value={"Techtribe Pay"}
                            checked={"Techtribe Pay" === subOrder?.methodPayment}
                            onChange={() => handleSelectPayment("Techtribe Pay")}
                            className="form-radio h-4 w-4 text-blue-600 transition duration-150 ease-in-out cursor-pointer"
                        />
                        ...
                        <label htmlFor={"paymentMethod"} className="ml-3 text-sm font-medium text-gray-700 flex gap-2 items-center">
                            ...
                            <p>Techtribe Pay</p>
                            {isNotAvaiLable('Techtribe Pay') && <p className="text-red-500">Số dư không đủ</p>}
                        </label>
                        
                        <div className="bg-primary/50 p-2 rounded-md backdrop-blur-sm flex items-center gap-2">

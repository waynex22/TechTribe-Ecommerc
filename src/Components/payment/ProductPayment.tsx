...
const ProductPayment: React.FC = () => {
    ...
    const { data: cart } = useGetCartMeQuery(user?.sub);
    ...
    const ListItemSelecte = cart?.cartItems.filter((item: any) => selectedProductIds?.includes(item.productPriceId._id));
    // console.log(ListItemSelecte);
    return (
        <>
            <div className="p-4 border border-gray-200 h-fit rounded-lg">
                <div className="flex items-start justify-between">
                    <div className="w-[70%]">
                        {ListItemSelecte && ListItemSelecte.map((item: any, index: number) => (
                            <div key={index} className="">
                                <div className="flex items-center justify-between p-2 gap-x-2 w-full" >

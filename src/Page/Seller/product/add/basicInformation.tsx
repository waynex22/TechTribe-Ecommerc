    ...
    <div id='BasicInformation' className='px-4 py-8 bg-white rounded-md shadow text-left'>
        ...
        <div className=' flex flex-col gap-6 pt-4 px-12 text-sm font-normal'>
            ...
            <div className=' flex gap-4 items-center'>
                ...
                <div className=' flex-1'>
                    <input onClick={()=> handlePopup()} type="text" className=" cursor-pointer border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-0 focus:border-gray-300 block w-full p-2.5" placeholder='Chọn danh mục' readOnly/>
                </div>
            </div>
            <div className=' flex gap-4 '>
                <div className=' w-60 text-right'>
        ...
        {!showCategory &&<Popup onHandlePopup={handlePopup}>
            <ShowCategory onHandlePopup={handlePopup} />
        </Popup>}
    </div>
  )
}
